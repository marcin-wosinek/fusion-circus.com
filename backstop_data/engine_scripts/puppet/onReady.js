  ...
  await page.evaluate(() => {
    const lazyImages = Array.from(
      document.querySelectorAll(
        'img[loading="lazy"], img[data-src], img[data-srcset], img.lazy, img.lazyload, img[data-lazy-src]',
      ),
    );
    ...
    const lazyBackgrounds = Array.from(
      document.querySelectorAll(
        "[data-bg], [data-background], [data-background-src]",
      ),
    );

    console.log(
      `Found ${lazyBackgrounds.length} potentially lazy-loaded background images`,
    );
